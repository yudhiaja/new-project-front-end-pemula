...
const addBook = () => {
  ...
  const book = makeBook(inputJudul, inputPengarang, inputTahun, false)
  const bookObject = composeBookObject(inputJudul, inputPengarang, inputTahun, false)
...
const makeBook = (judul, pengarang, tahun, isCompleted) => {
  const image = document.createElement('img');
  if(isCompleted) {
    image.setAttribute('src', 'assets/img/Read1.jpg')
  } else {
    image.setAttribute('src', 'assets/img/Unread1.jpg')
  }
  ...
  bookJudul.innerText = judul;
  
  const NamaPengarang = document.createElement('p');
  NamaPengarang.innerText = pengarang;
  
  const bookTahun = document.createElement('small');
  bookTahun.innerText = `${tahun}`;
  
  const detail = document.createElement('div');
  detail.classList.add('detail-book')
  detail.append(bookJudul, NamaPengarang, bookTahun)
...
const addBookToCompleted = (bookElement) => {
  const bookCompleted = document.getElementById(COMPLETED_BOOK_ID);
	...
	const bookJudul = bookElement.querySelector(".detail-book > h3").innerText;
  const bookPengarang = bookElement.querySelector(".detail-book > p").innerText;
  const bookTahun = bookElement.querySelector(".detail-book > small").innerText;
 
  const newBook = makeBook(bookJudul, bookPengarang, bookTahun, true);
...
const undoBookFromCompleted = (bookElement) => {
  ...
  const newBook = makeBook(bookJudul, bookPengarang, bookTahun, false);
  const book = findBook(bookElement[BOOK_ITEMID]);
